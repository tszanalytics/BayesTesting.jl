...
plot(dfwk1[:,1], st=:density, linecolor = "green" , label="wk0",gridcolor="grey", fill=(0,:green),alpha=0.3,title="SSRIs, T-Pbo",legend=:topleft)
...
savefig("SSRIdiffs12wks.png")

# p-values and odds
println([bayespval(dfwk1[:,1]) bayespval(dfwk1[:,2]) bayespval(dfwk1[:,3]) bayespval(dfwk1[:,4])])

println([mcodds(dfwk1[:,1]) mcodds(dfwk1[:,2]) mcodds(dfwk1[:,3]) mcodds(dfwk1[:,4])])
...
traj_plot(m,nwks=4,overlay=false,ci=false)
#traj_plot(m,nwks=3,overlay=true,ci=false)

z = [1 2 3 4]'
scatter!(z',m',label="")

savefig("SSRItraj12wks.png")
